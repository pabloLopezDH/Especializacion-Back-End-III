...
func main() {
	...
	repoTurn := turn.NewRepository(storage)
	serviceTurn := turn.NewService(repoTurn)
	turnHandler := handler.NewTurnHandler(serviceTurn)
	turn := r.Group("/turn")
	{
		turn.GET(":id", turnHandler.GetTurnByID())
		turn.POST("", turnHandler.Post())
		turn.PUT(":id", turnHandler.Put())
		turn.PATCH(":id", turnHandler.Patch())
		turn.DELETE(":id", turnHandler.Delete())
		turn.POST("/createturn", turnHandler.PostCreateTurn())
		turn.GET("/patient/:dni", turnHandler.GetTurnByDniPatient())
	}
	r.Run(":8080")
