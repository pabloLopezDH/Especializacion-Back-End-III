...
type Service interface {
	...
	GetByParams(searchParams url.Values) ([]domain.Product, error)
}
...
func (s *service) GetByParams(searchParams url.Values) ([]domain.Product, error) {
	if len(searchParams) == 0 {
		allProd, err := s.r.GetAll()
		if err != nil {
			return nil, err
		}
		return allProd, nil
	}
	productFinded, err := s.r.Search(searchParams)
	if err != nil {
		return nil, err
	}
